  ...
  return <Shell>
      <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          {statCards.map(s => {
          const Icon = s.icon;
          return <motion.div key={s.label} variants={item}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <motion.div variants={item} className="lg:col-span-2 space-y-6">
            ...
            <Card className="bg-card/40 backdrop-blur border-border/50">
              ...
              <CardContent className="space-y-2">
                {stats.topRiskyStocks.map((stock, idx) => <Link key={stock.ticker} href={`/stock/${stock.ticker}`}>
                    <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 border border-transparent hover:border-border/50 transition-all cursor-pointer group">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        background: stock.riskScore > 80 ? "hsl(var(--destructive))" : stock.riskScore > 50 ? "#f59e0b" : "hsl(var(--success))"
                      }} />
                        </div>
                      </div>
            ...
            <Card className="bg-card/40 backdrop-blur border-border/50">
              ...
              <CardContent className="space-y-3">
                {stats.recentAlerts.map(alert => <div key={alert.id} className="p-3 rounded-xl bg-black/20 border border-border/30 hover:border-border/60 transition-colors">
          ...
          <motion.div variants={item} className="space-y-6">
            ...
            <Card className="bg-card/40 backdrop-blur border-border/50">
              ...
              <CardContent>
                {!watchlist || watchlist.length === 0 ? <div className="text-center py-8 text-muted-foreground text-sm">
                    <Star className="w-8 h-8 mx-auto mb-2 opacity-20" />
                    Search a stock to add it here.
                  </div> : <div className="space-y-2">
                    {watchlist.map(wl => <Link key={wl.id} href={`/stock/${wl.ticker}`}>
                        <div className="flex items-center p-3 rounded-xl bg-black/20 border border-border/30 hover:border-primary/40 transition-all cursor-pointer relative group">
                          ...
                          <Button variant="ghost" size="icon" className="absolute top-1.5 right-1.5 w-6 h-6 opacity-0 group-hover:opacity-100 bg-background/80 hover:bg-destructive/20 hover:text-destructive transition-all" onClick={e => handleRemoveWatchlist(wl.id, e)} disabled={removeWatchlist.isPending}>
