...
import { Settings, Bell, Shield, Moon, Sliders, Info } from "lucide-react";
...
function Toggle({
  ...
  return <button onClick={() => onChange(!value)} className={`relative w-11 h-6 rounded-full transition-colors duration-300 ${value ? "bg-primary" : "bg-border"}`}>
...
function RangeSlider({
  ...
  return <input type="range" min={min} max={max} value={value} onChange={e => onChange(Number(e.target.value))} className="w-full h-2 rounded-full appearance-none cursor-pointer" style={{
        ...
        <motion.div variants={item}>
          <Card className="bg-card/40 backdrop-blur border-border/50">
            ...
            <CardContent className="space-y-4">
              ...
              desc: "Immediate alerts for critical fraud detections (risk score 90+)"
            }, {
              key: "highRiskAlerts",
              label: "High Risk Alerts",
              desc: "Alerts for high-risk events (risk score 70–89)"
            }, {
              key: "volumeSpikes",
              label: "Volume Spike Detection",
              desc: "Notify when abnormal trading volume is detected"
            }, {
              key: "sentimentShifts",
              label: "Sentiment Shifts",
              desc: "Alert on major sentiment changes in tracked stocks"
            }, {
              key: "weeklyDigest",
              label: "Weekly Digest",
              desc: "A summary of the week's top fraud detections"
            }].map(({
              key,
              label,
              desc
                  ...
                  <Toggle value={notifications[key]} onChange={v => setNotifications(prev => ({
                ...prev,
                [key]: v
              }))} />
                </div>)}
        ...
        <motion.div variants={item}>
          <Card className="bg-card/40 backdrop-blur border-border/50">
            ...
            <CardContent className="space-y-5">
              ...
              <div className={`p-3 rounded-xl text-xs border ${riskThreshold >= 80 ? "bg-destructive/10 border-destructive/20 text-destructive" : riskThreshold >= 50 ? "bg-amber-500/10 border-amber-500/20 text-amber-400" : "bg-emerald-500/10 border-emerald-500/20 text-emerald-400"}`}>
                {riskThreshold >= 80 ? "⚠️ High threshold — only critical manipulation events will trigger alerts." : riskThreshold >= 50 ? "⚡ Moderate threshold — medium and high risk events will trigger alerts." : "✅ Low threshold — most suspicious activity will trigger alerts."}
              </div>
            </CardContent>
          </Card>
        </motion.div>
