...
export default function StockAnalysis() {
  ...
  const { data: stock, isLoading: stockLoading } = useGetStock(ticker || "", {
  ...
  const { data: sentiment, isLoading: sentimentLoading } = useGetStockSentiment(ticker || "", {
    query: { enabled: !!ticker, queryKey: getGetStockSentimentQueryKey(ticker || "") }
  });
  ...
  const toggleWatchlist = () => {
    if (isWatched) {
      ...
      addToWatchlist.mutate({ data: { ticker: stock.ticker, stockName: stock.name } }, {
        onSuccess: () => queryClient.invalidateQueries({ queryKey: getListWatchlistQueryKey() })
      });
    }
  };
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <Button
                ...
                className={isWatched ? "bg-primary/20 text-primary border-primary/30" : ""}
              >
                {addToWatchlist.isPending || removeFromWatchlist.isPending ? (
                  <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                ) : (
                  <Star className={`w-4 h-4 mr-2 ${isWatched ? "fill-primary" : ""}`} />
                )}
                ...
                <div className="h-[300px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={stock.priceHistory} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                      ...
                      {stock.priceHistory.map((pt, i) => 
                        pt.anomaly ? (
                          <ReferenceDot key={i} x={pt.date} y={pt.close} r={4} fill="hsl(var(--destructive))" stroke="none" />
                        ) : null
                      )}
                    </AreaChart>
                  </ResponsiveContainer>
              ...
              <CardContent>
                <div className="mb-6">
                  <div className="flex justify-between items-end mb-2">
                    ...
                    <span className="capitalize font-bold text-lg" style={{ 
                      color: sentiment.overallSentiment === 'positive' ? 'hsl(var(--success))' : 
                             sentiment.overallSentiment === 'negative' ? 'hsl(var(--destructive))' : 'hsl(var(--muted-foreground))'
                    }}>
                  ...
                  <div className="h-3 w-full bg-black/40 rounded-full overflow-hidden relative">
                    ...
                    <div 
                      className="h-full bg-gradient-to-r from-destructive via-muted to-success"
                      style={{ width: '100%', opacity: 0.2 }}
                    />
                    <div 
                      className="absolute top-0 bottom-0 bg-primary shadow-[0_0_10px_rgba(0,212,255,0.8)]"
                      style={{ 
                ...
                <div className="space-y-4">
                  ...
                  {sentiment.headlines.map((hl, i) => (
                    <div key={i} className="flex flex-col md:flex-row md:items-center justify-between p-3 rounded bg-black/20 gap-2">
                      <div>
                      ...
                      <SeverityBadge 
                        severity={hl.sentiment === 'negative' ? 'critical' : hl.sentiment === 'positive' ? 'low' : 'medium'} 
                        className={hl.sentiment === 'neutral' ? 'bg-muted/20 text-muted-foreground border-border' : ''}
                      />
                    </div>
                  ))}
                </div>
          ...
          <div className="space-y-6">
            <Card className="bg-card/50 backdrop-blur border-border/50 text-center flex flex-col items-center justify-center p-6 py-8">
              ...
              <div className="mt-6 font-medium text-lg">Risk Level: <span className="capitalize" style={{
                color: analysis.riskLevel === 'critical' || analysis.riskLevel === 'high' ? 'hsl(var(--destructive))' :
                       analysis.riskLevel === 'medium' ? 'hsl(var(--warning))' : 'hsl(var(--success))'
