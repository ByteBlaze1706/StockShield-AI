...
const SECTOR_DATA = [
  { sector: "Consumer Disc.", risk: 87, stocks: 3, color: "#ef4444" },
  { sector: "Materials", risk: 68, stocks: 1, color: "#f59e0b" },
  { sector: "Technology", risk: 14, stocks: 3, color: "#22c55e" },
  { sector: "Comm. Services", risk: 37, stocks: 3, color: "#3b82f6" },
  { sector: "Consumer Staples", risk: 52, stocks: 1, color: "#f97316" },
  { sector: "ETF", risk: 5, stocks: 1, color: "#10b981" },
];

const FRAUD_TYPES = [
  { name: "Pump & Dump", count: 12, color: "#ef4444" },
  { name: "Volume Spike", count: 9, color: "#f59e0b" },
  { name: "Wash Trading", count: 7, color: "#8b5cf6" },
  { name: "Social Hype", count: 5, color: "#3b82f6" },
  { name: "Insider Signal", count: 3, color: "#06b6d4" },
];

const RISK_DISTRIBUTION = [
  { range: "0–20 (Safe)", count: 4, color: "#22c55e" },
  { range: "21–40 (Low)", count: 2, color: "#84cc16" },
  { range: "41–60 (Medium)", count: 1, color: "#f59e0b" },
  { range: "61–80 (High)", count: 1, color: "#f97316" },
  { range: "81–100 (Critical)", count: 4, color: "#ef4444" },
];

const RADAR_DATA = [
  { subject: "Volume", A: 82 },
  { subject: "Sentiment", A: 67 },
  { subject: "Price", A: 54 },
  { subject: "Social", A: 73 },
  { subject: "Insider", A: 38 },
  { subject: "Fundamentals", A: 45 },
];

const STOCK_COMPARISON = [
  { ticker: "BBBY", risk: 96, hype: 89, volume: 94, insider: 72 },
  { ticker: "MMAT", risk: 91, hype: 78, volume: 88, insider: 61 },
  { ticker: "GME", risk: 87, hype: 95, volume: 91, insider: 55 },
  { ticker: "AMC", risk: 82, hype: 88, volume: 84, insider: 48 },
  { ticker: "SNDL", risk: 78, hype: 65, volume: 79, insider: 41 },
  { ticker: "TSLA", risk: 61, hype: 55, volume: 58, insider: 33 },
  { ticker: "NVDA", risk: 22, hype: 42, volume: 30, insider: 15 },
  { ticker: "META", risk: 19, hype: 28, volume: 22, insider: 12 },
];
    ...
    <Shell>
      <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          {[
            { label: "Fraud Patterns Found", value: "36", icon: AlertTriangle, color: "text-destructive", glow: "shadow-[0_0_20px_-8px_hsl(var(--destructive))]" },
            { label: "High Risk Sectors", value: "2", icon: BarChart2, color: "text-amber-400", glow: "shadow-[0_0_20px_-8px_rgba(251,191,36,0.5)]" },
            { label: "Manipulation Score", value: "73/100", icon: Shield, color: "text-primary", glow: "shadow-[0_0_20px_-8px_hsl(var(--primary))]" },
            { label: "Trending Anomalies", value: "+4 today", icon: TrendingUp, color: "text-emerald-400", glow: "shadow-[0_0_20px_-8px_rgba(52,211,153,0.5)]" },
          ].map((c) => {
            const Icon = c.icon;
            return (
                ...
                <div className="h-56">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={SECTOR_DATA} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                      ...
                      <XAxis dataKey="sector" stroke="hsl(var(--muted-foreground))" fontSize={10} tick={{ fontSize: 10 }} />
                  ...
                  {RISK_DISTRIBUTION.map((r) => (
                    <div key={r.range} className="space-y-1">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-bold" style={{ color: r.color }}>{r.count} stocks</span>
                      </div>
        ...
        <motion.div variants={item}>
          <Card className="bg-card/40 backdrop-blur border-border/50">
            ...
            <CardContent>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={STOCK_COMPARISON} margin={{ top: 0, right: 0, left: -10, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" vertical={false} opacity={0.4} />
