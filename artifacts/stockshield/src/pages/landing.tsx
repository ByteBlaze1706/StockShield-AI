...
const FEATURES = [
  { icon: Activity, title: "Real-time Monitoring", desc: "Constant surveillance of price action and volume patterns for anomalies across 12+ monitored stocks.", color: "text-primary", bg: "bg-primary/10 border-primary/20" },
  { icon: Lock, title: "AI Risk Analysis", desc: "Gemini AI performs deep evaluation of fundamental and technical risk factors to score each asset.", color: "text-secondary", bg: "bg-secondary/10 border-secondary/20" },
  { icon: Eye, title: "Sentiment Tracking", desc: "Natural language processing detects coordinated pump schemes across news headlines in real time.", color: "text-amber-400", bg: "bg-amber-400/10 border-amber-400/20" },
  { icon: TrendingUp, title: "Volume Spike Alerts", desc: "Automatic detection of unusual trading volume that may signal wash trading or coordinated buying.", color: "text-emerald-400", bg: "bg-emerald-400/10 border-emerald-400/20" },
  { icon: AlertTriangle, title: "Insider Activity Signals", desc: "Pattern recognition to identify potential insider trading before information becomes public.", color: "text-orange-400", bg: "bg-orange-400/10 border-orange-400/20" },
  { icon: Bot, title: "ShieldBot Assistant", desc: "Your personal AI analyst explains fraud risks in plain English — no finance degree required.", color: "text-violet-400", bg: "bg-violet-400/10 border-violet-400/20" },
];

const HOW_IT_WORKS = [
  { step: "01", title: "Monitor the Market", desc: "StockShield continuously tracks price action, volume, and social sentiment for suspicious activity." },
  { step: "02", title: "AI Detection Engine", desc: "Our Gemini-powered engine analyzes patterns and scores each stock's fraud risk from 0 to 100." },
  { step: "03", title: "Instant Alerts", desc: "When manipulation is detected, you get a detailed alert explaining the threat — before it's too late." },
  { step: "04", title: "Protect Your Portfolio", desc: "Use our AI chat assistant to get plain-language explanations and actionable recommendations." },
];

const TESTIMONIALS = [
  { name: "Sarah K.", role: "Retail Investor", text: "StockShield caught a pump-and-dump scheme on MMAT before I invested. Saved me thousands.", avatar: "SK" },
  { name: "Marcus T.", role: "Day Trader", text: "The volume spike detection is incredibly accurate. It flagged GME activity 2 hours before the mainstream news.", avatar: "MT" },
  { name: "Priya R.", role: "Financial Student", text: "ShieldBot explains fraud patterns in a way I can actually understand. Best learning tool for beginners.", avatar: "PR" },
];

const STATS = [
  { value: "12+", label: "Stocks Monitored" },
  { value: "94%", label: "AI Accuracy" },
  { value: "2h", label: "Avg. Early Warning" },
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <section className="relative max-w-7xl mx-auto px-4 md:px-6 pt-24 pb-20 text-center">
        ...
        >
          {STATS.map((s) => (
            <div key={s.label} className="text-center p-4 rounded-xl bg-card/40 border border-border/50 backdrop-blur-sm">
        ...
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-80px" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {FEATURES.map((f) => {
              const Icon = f.icon;
              return (
                <motion.div key={f.title} variants={fadeUp} className="p-6 rounded-2xl bg-card/40 border border-border/40 backdrop-blur-sm hover:border-border/80 transition-all group hover:scale-[1.01]">
      ...
      <section id="testimonials" className="max-w-7xl mx-auto px-4 md:px-6 py-20">
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-80px" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {TESTIMONIALS.map((t) => (
              <motion.div key={t.name} variants={fadeUp} className="p-6 rounded-2xl bg-card/40 border border-border/40 backdrop-blur-sm hover:border-border/80 transition-all">
      ...
      <section className="max-w-4xl mx-auto px-4 md:px-6 py-20">
        ...
        >
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-4 text-xs text-muted-foreground">
            {["Free forever", "No credit card", "AI-powered", "Real-time alerts"].map(item => (
