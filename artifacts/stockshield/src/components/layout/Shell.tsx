...
const TICKER_DATA = [
  { ticker: "AAPL", price: 189.45, change: 0.65 },
  { ticker: "TSLA", price: 248.79, change: -2.13 },
  { ticker: "GME", price: 14.22, change: 32.0 },
  { ticker: "NVDA", price: 875.40, change: 1.82 },
  { ticker: "AMC", price: 4.87, change: 29.8 },
  { ticker: "MSFT", price: 415.32, change: 0.52 },
  { ticker: "META", price: 512.67, change: 0.85 },
  { ticker: "MMAT", price: 0.34, change: 35.9 },
  { ticker: "SPY", price: 521.43, change: 0.36 },
  { ticker: "BBBY", price: 0.11, change: 37.5 },
  { ticker: "GOOGL", price: 171.23, change: 0.52 },
  { ticker: "SNDL", price: 1.74, change: 31.8 },
];

function TickerTape() {
  const doubled = [...TICKER_DATA, ...TICKER_DATA];
  return (
    <div className="w-full overflow-hidden bg-black/40 border-b border-border/30 py-1.5 relative">
      ...
      <nav className="flex-1 py-4 px-2 space-y-0.5 overflow-y-auto">
        ...
        {navItems.map((navItem) => {
          ...
          return (
            ...
            >
              {isActive && (
                <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-primary rounded-r-full shadow-[0_0_8px_2px_hsl(var(--primary))]" />
              )}
              ...
              {navItem.label === "ShieldBot" && (
                <span className="ml-auto text-[9px] bg-primary/20 text-primary border border-primary/30 px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider">AI</span>
              )}
            </Link>
          );
        })}
