...
const STOCK_RISK_DATA = {
  ...
  TSLA: {
    riskScore: 61,
    riskLevel: "medium",
    flags: ["high_volatility", "social_media_influence"]
  },
  GME: {
    riskScore: 87,
    riskLevel: "high",
    flags: ["volume_spike", "short_squeeze", "retail_speculation", "social_media_driven"]
  },
  AMC: {
    riskScore: 82,
    riskLevel: "high",
    flags: ["volume_spike", "pump_and_dump_pattern", "high_retail_activity"]
  },
  MSFT: {
    riskScore: 8,
    riskLevel: "safe",
  ...
  BBBY: {
    riskScore: 96,
    riskLevel: "critical",
    flags: ["bankruptcy_risk", "volume_spike", "pump_and_dump_pattern", "short_squeeze", "delisting_risk"]
  },
  SNDL: {
    riskScore: 78,
    riskLevel: "high",
    flags: ["volume_spike", "penny_stock", "regulatory_scrutiny"]
  },
  META: {
    riskScore: 19,
    riskLevel: "low",
  ...
  MMAT: {
    riskScore: 91,
    riskLevel: "critical",
    flags: ["pump_and_dump_pattern", "volume_spike", "SEC_investigation", "penny_stock"]
  },
  SPY: {
    riskScore: 5,
    riskLevel: "safe",
...
function getDefaultRisk(ticker) {
  return {
    riskScore: 45,
    riskLevel: "medium",
...
Only return valid JSON, no markdown, no extra text.`;
    ...
    res.json({
      ...
      riskFactors: risk.flags.map(f => ({
        label: f.replace(/_/g, " "),
        severity: risk.riskScore > 70 ? "high" : "medium",
        description: `This stock exhibits ${f.replace(/_/g, " ")} patterns that warrant caution.`
      })),
...
router.get("/:ticker/sentiment", async (req, res) => {
  ...
  const mockHeadlines = {
    GME: [{
      title: "GameStop surges 200% as Reddit traders pile in again",
      source: "CNBC",
      sentiment: "negative"
    }, {
      title: "SEC launches inquiry into GameStop trading surge",
      source: "Wall Street Journal",
      sentiment: "negative"
    }, {
      title: "GameStop short sellers face $2.8B in losses",
      source: "Bloomberg",
      sentiment: "neutral"
    }, {
      title: "Roaring Kitty returns: GameStop mania reignites",
      source: "Reuters",
      sentiment: "negative"
    }],
    BBBY: [{
      title: "Bed Bath & Beyond files for bankruptcy protection",
      source: "Reuters",
      sentiment: "negative"
    }, {
      title: "BBBY stock surges 300% despite bankruptcy filing",
      source: "CNBC",
      sentiment: "negative"
    }, {
      title: "SEC warns investors about BBBY trading risks",
      source: "MarketWatch",
      sentiment: "negative"
    }],
    AAPL: [{
      ...
      title: "iPhone 16 demand exceeds analyst expectations",
      source: "Wall Street Journal",
      sentiment: "positive"
    }, {
      title: "Apple Vision Pro showing strong enterprise adoption",
      source: "Reuters",
      sentiment: "positive"
    }],
    TSLA: [{
      title: "Tesla deliveries miss Q3 expectations",
      source: "CNBC",
      sentiment: "negative"
    }, {
      title: "Elon Musk tweets send Tesla shares volatile",
      source: "Bloomberg",
      sentiment: "negative"
    }, {
      ...
      title: "Tesla FSD beta gets regulatory scrutiny",
      source: "Wall Street Journal",
      sentiment: "neutral"
    }]
  };
  const headlines = mockHeadlines[ticker] ?? [{
    title: `${ticker} trading volume remains stable`,
    source: "MarketWatch",
    sentiment: "neutral"
  }, {
    ...
    title: `${ticker} reports quarterly earnings in line with estimates`,
    source: "Reuters",
    sentiment: "positive"
  }];
  try {
